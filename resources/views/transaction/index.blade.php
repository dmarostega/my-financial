                    ...
                    <div>
                        ...
                        @if($transaction->transactionParts->first() && !filled($transaction->transactionParts->first()->payment_date))
                            @if($transaction->bill &&  $transaction->bill->type == 'to_pay'  && !$transaction->card)                        
                                <x-link :href="route('resolving.confirm',['id' => $transaction->transactionParts->first()->id, 'transaction'])">
                                    {{ __('Pay') }}
                                </x-link>
                            ...
                            @endif 
                        @else 
                            <x-link :href="route('resolving.confirm', [
                                                                        'id' => $transaction->transactionParts->first()->id, 'extort'])">
                                {{ __('extort') }}
                            </x-link>
                        @endif           
                    </div>
